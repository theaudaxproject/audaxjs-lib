...
describe('swiftcashjs-lib (BIP32)', function () {
  it('can import a BIP32 testnet xpriv and export to WIF', function () {
    ...
    var node = swiftcash.HDNode.fromBase58(xpriv, swiftcash.networks.testnet)
  ...
  it('can export a BIP32 xpriv, then import it', function () {
    ...
    var node = swiftcash.HDNode.fromSeedBuffer(seed)
    var string = node.toBase58()
    var restored = swiftcash.HDNode.fromBase58(string)
  ...
  it('can create a BIP32, swiftcash, account 0, external address', function () {
    var path = "m/0'/0/0"
    var root = swiftcash.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, swiftcash, account 0, external address', function () {
    var root = swiftcash.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP49, swiftcash testnet, account 0, external address', function () {
    var mnemonic = 'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about'
    var seed = bip39.mnemonicToSeed(mnemonic)
    var root = swiftcash.HDNode.fromSeedBuffer(seed)

    var path = "m/49'/1'/0'/0/0"
    var child = root.derivePath(path)

    var keyhash = swiftcash.crypto.hash160(child.getPublicKeyBuffer())
    var scriptSig = swiftcash.script.witnessPubKeyHash.output.encode(keyhash)
    var addressBytes = swiftcash.crypto.hash160(scriptSig)
    var outputScript = swiftcash.script.scriptHash.output.encode(addressBytes)
    var address = swiftcash.address.fromOutputScript(outputScript, swiftcash.networks.testnet)
